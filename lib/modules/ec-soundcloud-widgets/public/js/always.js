apos.define('ec-soundcloud-widgets', {
    ...
    construct: function(self, options) {
        self.play = function ($widget, data, options) {
            ...
            if(!location.includes('?track')){ // DIRECTORY VIEW RENDER
                $.get('/modules/ec-soundcloud-widgets/soundcloud?api_key='+data.api_key+'&client_id='+encodeURIComponent(data.api_user), function (results) {
                    ...
                    $.get("https://api.soundcloud.com/users/"+id+"/tracks?client_id="+encodeURIComponent(data.api_key) , function (result) {
                        ...
                        $("#tracks > tr[id]").each(function(){
                            this.onclick = function() { trackFocus(this.id) };
                        });

                        $("#feature-container > div[id]").each(function(){
                            this.onclick = function() { trackFocus(result[this.id[this.id.length - 1]].id) };
            ...
            }else{ // FOCUS VIEW RENDER
                var track_id = location.split("track=");
                var id = data.client_id;
                $("#podcast-nav > div[id").each(function(){
                    if (this.id == 'focus-backbtn'){
                        this.onclick = function() {
                            $.get("https://api.soundcloud.com/users/"+id+"/tracks?client_id="+encodeURIComponent(data.api_key) , function (result) {
                                var newid = findPrev(track_id[1], result);
                                if (newid != null){
                                    trackFocus(newid);
                                }
                            }); 
                        };
                    }else if (this.id == 'focus-forwardbtn'){
                        this.onclick = function() {
                            $.get("https://api.soundcloud.com/users/"+id+"/tracks?client_id="+encodeURIComponent(data.api_key) , function (result) {
                                var newid = findNext(track_id[1], result);
                                if (newid != null){
                                    trackFocus(newid);
                                }
                            }); 
                ...
                $.get("https://api.soundcloud.com/tracks/"+ track_id[1] +"?client_id="+encodeURIComponent(data.api_key), function (result) {
                    ...
                    $widget.find('#track-name')[0].textContent = result.title;
                    $widget.find('#track-desc')[0].textContent = result.description;
                    $widget.find('#track-player')[0].setAttribute('src',
