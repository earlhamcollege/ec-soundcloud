const debug = false; // set true for debug mode with console.logs enabled
apos.define('ec-soundcloud-widgets', {
    ...
    construct: function(self, options) {
        self.play = function ($widget, data, options) {
            ...
            $.get('/modules/ec-soundcloud-widgets/soundcloud?api_key='+data.api_key+'&client_id='+encodeURIComponent(data.user_name), function(results) {
                //console.log(results);
                //console.log(data);
                //console.log(widget);
                //console.log($widget);
                ...
                //console.log(res);
                //console.log(req);
                //console.log(results.soundcloud);
                ...
                $.get("//api.soundcloud.com/users/"+data.client_id+"?client_id="+encodeURIComponent(data.api_key), function(account_result) {
                    console.log(account_result);
                    $widget.find('.data-user').val('');
                    createHeader(account_result) // render header
                });
                // TRACK INFORMATION API CALL
                $.get("//api.soundcloud.com/users/"+data.client_id+"/tracks?client_id="+encodeURIComponent(data.api_key), function(tracks_result) {
                    if (debug) { console.table(tracks_result); }
            ...
            if (debug) { console.log('render complete'); }

            // Render the content area of the widget
            function renderContent(tracks, data){
                if (debug) { console.info('creating content'); }

                var location = window.location.href;

                if(!location.includes('?track')){
                    //no track in url                console.info('render feature tracks');
            ...
            function renderFeature(tracks, data){
                if (debug) { console.info('content = features'); }
                ...
                for(x = 0; x < data.feature_count; x++){
                    ...
                    if (debug) { console.log(tracks); console.log(tracks[x].id); }
            ...
            function renderFocus(tracks, data){
                if (debug) { console.info('content = focus'); }
                ...
                $.get("//api.soundcloud.com/tracks/"+ track_id[1] +"?client_id="+encodeURIComponent(data.api_key), function(result) {
                    ...
                    $widget.find('#track-player')[0].setAttribute('src',"//w.soundcloud.com/player/?url=https%3A//api.soundcloud.com/tracks/" + result.id + "&color=" + data.player_color.split('#')[1] + "&auto_play=" + data.player_autoplay + "&hide_related=false" + "&show_comments=" + data.player_comments + "&show_user=true" + "&show_reposts=false" + "&show_teaser=true");
                    $widget.find('#track-player')[0].id = track_id[1];
                });
            }
            // create header with account information
            function createHeader(result){
                if (debug) { console.info('creating header'); }
                container = $widget.find('.data-user');

                container.onclick = function(){
                    if ('track' in window.location){
                        if (debug) { console.log('return to features'); }
                ...
                if (data.toggle_account_desc == false){
            ...
            function createTable(result, data){
                if (debug) { console.info('creating table'); }
                ...
                track += '<td><strong>' + "Title" + '</strong></td>'; //defaults
                track += '<td><strong>' + "Date" + '</strong></td>';
                track += '<td><strong>' + "Duration" + '</strong></td>';
                if (prefArr.includes('views')){     // view count
                    track += '<td><strong>' + "View Count" + '</strong></td>';
                }
                if (prefArr.includes('favorites')){     // fave count
                    track += '<td><strong>' + "Favorite Count" + '</strong></td>';
                }
                track += "</tr>";
                ...
                for(var i = 0; i < ep_count; i++){
                    ...
                    track += '<td>' + result[i].title + '</td>'; // defualts
                    ...
                    if (prefArr.includes('favorites')){     // fave count
                        track += '<td>' + result[i].favoritings_count + '</td>';
                    }
                    track += "</tr>";
                }
...
function returnToFeatures(){
    if (debug) { console.info('return to features'); }
