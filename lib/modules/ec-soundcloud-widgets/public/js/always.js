    ...
    construct: function(self, options) {
        self.play = function ($widget, data, options) {
            ...
            function createTable(result, data){
                ...
                var track = '';
                prefArr = [data.table_col_count, data.table_col_title, data.table_col_date, data.table_col_time, data.table_col_views];
                console.log(prefArr);

                // CREATE TABLE HEADERS //
                track += "<tr id='table-header'>";
                if (prefArr[0] == true){     // episode count
                    track += '<td><strong>' + "Episode" + '</strong></td>';
                }
                if (prefArr[1] == true){     // title
                    track += '<td><strong>' + "Title" + '</strong></td>';
                }
                if (prefArr[2] == true){      // date
                    track += '<td><strong>' + "Date" + '</strong></td>';
                }
                if (prefArr[3] == true){      // time
                    track += '<td><strong>' + "Time" + '</strong></td>';
                }
                if (prefArr[4] == true){     // view count
